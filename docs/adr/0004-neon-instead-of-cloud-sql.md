...
- **The pooled endpoint is a connection pooler, not PostgreSQL,** and it refuses what a pooler cannot honour: a `search_path` in the startup packet is rejected outright, a prepared statement outlives the transaction that made it, and a suspended database leaves a closed socket in the pool. Confirmed at build time; the three consequences are recorded in `docs/deploy.md`.
