...
The tests above prove the code does what it was written to do. The eval gate is
the other question: does the Coach still answer well. About sixty hand-written
cases — `agent/evals/`, and `agent/evals/README.md` is the whole of it — run
against the real graph on every pull request.

| Group | Count | Scored by |
|---|---|---|
| safety | 22 | plain assertions, never a model |
| retrieval | 21 | assertions, and ragas on the prose |
| behaviour | 21 | plain assertions |

A case asserts structure and never wording: the Intents the router chose, that a
Refusal fired with its disclaimer and its referral, that the citations list is
not empty and names the expected document, that a named allergen appears nowhere,
that the schema validated, that the logged Items match. The prose is judged by
ragas alone, so a harmless rewording cannot fail a build.

**A missed Refusal, a missing Citation, an allergen in an answer, or a schema
failure always blocks.** The four ragas means block only on a fall of more than a
set margin below a stored baseline, and the baseline ratchets upwards when a
change improves it — a fixed pass mark would train people to re-run the pipeline
until it passed.

**No case is generated by a model**, and the safety cases stay hand-written for
ever: a generated expected answer can encode a mistake as the correct behaviour.
**There is no quarantine or skip list**, and a case carrying one of the words such
a thing would arrive under is a load error.

**A behaviour change edits its case in the same pull request**, so a reviewer sees
the claim next to the evidence. **A safety case change additionally needs a
written reason in the pull request body**, saying why the boundary moved; the
pull-request template asks for it.

```sh
cd agent
DATABASE_URL=... GOOGLE_API_KEY=... python -m evals.run --rows rows.json
# the judge, in an environment of its own: ragas 0.4 cannot sit beside langchain 1.x
python -m venv .judge && .judge/bin/pip install -r evals/requirements-judge.txt
.judge/bin/python -m evals.judge rows.json
```
